<?php
	...
	class Registro{
		...
		public function registrarUsuario($perfil, $pass){
			...
			if($usr = pg_fetch_array($verif)) echo("El nombre de usuario ya existe en el sistema");
			else{
				$regUsr = "INSERT INTO Usuario (nombre_usuario, contrasena, vigente, perfil_idperfil) values ('$this->usuario', '$pass', '1', '$perfil')";
				if($this->validaRegistro($cx, $regUsr)) echo("Registro de usuario exitoso");
				else echo("Error al registrar al usuario ".$this->usuario."con contraseña ".$pass);
			}
