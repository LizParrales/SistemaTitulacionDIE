<?php
	...
	if(isset($_POST['ssocial'])){
		...
		if(!empty($_FILES['ssocial_arch']['name']) && $_POST['ssocial'] != 'falta'){
			$dir = './tmp/';
			...
			if($regAluSS = pg_fetch_array(pg_query("SELECT id_ss FROM alumno_ss WHERE alumno_no_cuenta = ".$_SESSION['cuenta'])))
				$sqlSSa = "UPDATE alumno_ss SET nombre = '$nombre', archivo_ss = '$oid', tipo_mime = '$tipo', size = '$size' WHERE alumno_no_cuenta = ".$_SESSION['cuenta'];
	...
	if(isset($_POST['ingles'])){
		...
		if(!empty($_FILES['ingles_arch']['name'])){
			...
			$eEn = pg_query("SELECT id FROM alumno_ingles WHERE vigente = '1' AND alumno_no_cuenta = ".$_SESSION['cuenta']);
			if($existeEn = pg_fetch_array($eEn)){
				$sqlEnBaja = "UPDATE alumno_ingles SET vigente = '0' WHERE alumno_no_cuenta = ".$_SESSION['cuenta'];
				pg_query($sqlEnBaja) or die(pg_last_error($cx));
			}
			
			$sqlEn = "INSERT INTO alumno_ingles (nombre, archivo, anio_exp, tipo_mime, size, alumno_no_cuenta) VALUES ('$nombre', '$oid', '$anio', '$tipo', '$size', '".$_SESSION['cuenta']."')";
			pg_query($cx, $sqlEn);
