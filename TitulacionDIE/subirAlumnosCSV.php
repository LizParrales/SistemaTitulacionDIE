<?php
	...
	if(!empty($_FILES['fileCSV']['name'])){
		...
		while(!feof($archivo)){
			...
			//echo($contenido[$cuenta]);
			if(!is_null($contenido[0])){
				
				$usr = pg_query("SELECT fecha_nac FROM alumno WHERE no_cuenta = ".$contenido[$cuenta]) or die(pg_last_error($cx));		
				
				if($usuarios = pg_fetch_array($usr)){
					//echo($usuarios['fecha_nac']);
