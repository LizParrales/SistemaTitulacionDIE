<?php
	...
	if(isset($_POST['carrera']))
		if(strlen($_POST['carrera']) > 0) $carreraSql = " AND carrera_clave = '".$_POST['carrera']."'";
	
	
	$fpCSV = fopen("alumnos.csv","w");
	fwrite($fpCSV, "Nombre,No. Cuenta,Semestre de ingreso,Avance de creditos,Promedio,Carrera,Correo,Telefono,Servicio social,Inglés,Opcion de titulacion".PHP_EOL);
	...
	if(isset($_SESSION['cargo'])){
		if($_SESSION['cargo'] != 'Jefe de Division'){
			...
			if((strlen($_POST['opTitulacion']) > 0)&&($_POST['opTitulacion'] == 't')){
				$sqlAlu = pg_query("SELECT * FROM alumno INNER JOIN alumno_opcion_titulacion ON alumno.no_cuenta = alumno_opcion_titulacion.alumno_no_cuenta WHERE alumno.vigente = '1'".$sqlSocial.$sqlEng.$carreraSql);
				$OpTrue = 1;
			}
			else{
				if((strlen($_POST['opTitulacion']) > 0)&&($_POST['opTitulacion'] == 'f')) $OpTrue = 0;
				else $OpTrue = 1;
				$sqlAlu = pg_query("SELECT * FROM alumno WHERE vigente = '1' ".$sqlSocial.$sqlEng.$carreraSql);
		...
		if($aluRes = pg_fetch_all($sqlAlu)){
			...
			foreach($aluRes as $v){
				...
				if(($v['avance_creditos'] >= $inicio)&&($v['avance_creditos'] <= $limite)){
					...
					if($v['servicio_social'] == 'falta') $ssocialCSV = "--";
					else $ssocialCSV = $v['servicio_social'];
					
					if($v['comp_ingles'] == 'f') $compEnCSV = "--";
					else $compEnCSV = "Cubierto";
					
					$renglonAlu .= "{$v['correo']},{$v['telefono']},{$ssocialCSV},{$compEnCSV},";
					...
					$opT = pg_query("SELECT opcion_titulacion.nombre, alumno_opcion_titulacion.comentario FROM opcion_titulacion, alumno_opcion_titulacion WHERE alumno_opcion_titulacion.opcion_titulacion_clave = opcion_titulacion.clave AND alumno_opcion_titulacion.vigente = '1' AND alumno_opcion_titulacion.alumno_no_cuenta =".$v['no_cuenta']);
					if($res = pg_fetch_array($opT)){
						$opcionT = $res['nombre']." ".$res['comentario'];
						$opRegExiste = 1;
					}
					else{
						$opcionT = "--";
