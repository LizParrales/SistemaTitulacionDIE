...
<body>
	...
	<div id="contenido">
		...
		<?php
			if(isset($_SESSION['primer_acceso'])&&($_SESSION['primer_acceso'] == '1')){
				echo("<div id=\"cambio_pass\" class=\"modal\">");
    				echo("<div>");
    					echo("<span id=\"cerrar\" class=\"cerrar close\">X</span>");
    					echo("<h3>Cambia tu contrase&ntilde;a</h3>");
    				echo("</div>");
    			echo("</div>");
    		}
		?>
		...
		<div id="info">
			<form enctype="multipart/form-data" method="post" action="modificarAlu.php" class="clase-form">
			
			<table class="table table-condensed table-bordered">
			<?php
				echo("<h1>".$_SESSION['usuarioAlu']."</h1>");
				...
				if($arr = pg_fetch_array($ca)){
					...
					if($arrSSArch = pg_fetch_array($ssArch)){
						...
						if(!is_null($arrSSArch['nombre'])){
							...
							echo("<br />");
							echo("Comentarios");
							echo("<br /><textarea name=\"ssocial_com\" cols=\"20\" rows=\"2\">");
							if(!is_null($arrSSArch['comentario'])) echo $arrSSArch['comentario'];
							echo("</textarea>");
						}
					}
					else{
					...
					$sqlEn = "SELECT id, anio_exp, archivo FROM alumno_ingles WHERE vigente = '1' AND alumno_no_cuenta = ".$_SESSION['cuenta']."ORDER BY anio_exp DESC";
					...
					if((!$arrEn['archivo'])||($anioEn >= 5)){
						...
						$idEn = $arrEn['id'];
						echo("<a href=\"archivoEn.php?id=$idEn&cuenta=".$_SESSION['cuenta']."\">Comprensi&oacute;n de lectura</a>");
					}
					echo("</td></tr>");
					echo("<tr>");
					echo("</table><table class=\"table table-condensed table-bordered\">");
					...
					$sqlOpAlu = pg_fetch_array(pg_query("SELECT opcion_titulacion_clave, comentario FROM alumno_opcion_titulacion WHERE vigente = '1' AND alumno_no_cuenta = ".$_SESSION['cuenta']));
					...
					echo("<br /><br />");
					echo("Comentarios");
					echo("<br /><textarea name=\"opTit_com\" cols=\"20\" rows=\"2\">");
					if(!is_null($sqlOpAlu['comentario'])) echo($sqlOpAlu['comentario']);
					echo("</textarea>");
					echo("</td></tr>");
					echo("<tr>");
				}
				...
				<input name='modificar' type='submit' value='Actualizar datos' class="btn btn-default" />
			</form>
		</div>
		<div id="footer">
		</div>
